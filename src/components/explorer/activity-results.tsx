        ...
        <div className="flex flex-col gap-3">
          {sorted.map(({ activity, matches }) => {
            ...
            return (
              <div
                key={activity.uri}
                className={`border rounded-lg p-4 transition-opacity duration-150 ${
                  expression && !matches
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  {expression && !matches && (
                    <span className="text-[10px] font-mono uppercase tracking-wider text-red-500 bg-red-50 border border-red-200 rounded px-1.5 py-0.5 flex-shrink-0">
                      Does not match
                    </span>
                  )}
                  {expression && matches && (
                    <span className="text-[10px] font-mono uppercase tracking-wider text-green-700 bg-green-50 border border-green-200 rounded px-1.5 py-0.5 flex-shrink-0">
                      Matches
                    </span>
                  )}
                </div>
